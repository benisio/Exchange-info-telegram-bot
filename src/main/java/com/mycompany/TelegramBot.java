...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    private static final String FIAT_SETTINGS_MESSAGE_TEXT = getCurrencySettingsMessageText("валютных");
    private static final String CRYPTO_SETTINGS_MESSAGE_TEXT = getCurrencySettingsMessageText("криптовалютных");
...
//    сохранялось от одного нажатия кнопки к другому.
        ...
        if (update.hasMessage() && update.getMessage().hasText()) {
            ...
            switch (callbackQueryData) {
                ...
                case DONT_SHOW_ALL_FIAT -> { // кнопка "не показывать все" в меню "отображение фиатных валют"
                    ...
                    editMessage(chatId, messageWithKeyboard.getMessageId(), keyboard); // error: message is not modified
                }
    ...
    private Message sendMessage(long userChatId, String text, InlineKeyboardMarkup keyboard) {
        try {
            return execute(SendMessage.builder().chatId(userChatId).text(text).replyMarkup(keyboard).build());
        } catch (TelegramApiException e) {
            e.printStackTrace();
    ...
    private InlineKeyboardMarkup createSettingsKeyboard() {
        List<InlineKeyboardButton> firstRow = List.of(createButton("отображение фиатных валют", FIAT_CURRENCIES_SETTINGS));
    ...
    private InlineKeyboardMarkup createCurrenciesKeyboard(List<CurrencyPair> currencyPairs, User user) {
        // из списка валютных пар создаем строки с кнопками, в каждой по две кнопки
        List<List<InlineKeyboardButton>> keyboardRows = new ArrayList<>();
        for (int i = 0; i < currencyPairs.size(); i += 2) {
            keyboardRows.add(createRowWithButtonsFor(currencyPairs.get(i), currencyPairs.get(i + 1), user));
        }
    ...
    private List<InlineKeyboardButton> createRowWithButtonsFor(CurrencyPair first, CurrencyPair second, User user) {
        return List.of(
